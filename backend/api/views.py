...
from .models import PurchaseLog, Portfolio, PortfolioCompany
from .serializers import PurchaseLogSerializer, PortfolioCompanyInputSerializer,PortfolioSerializer, PortfolioCompanySerializer
...
class PortfolioListCreateView(APIView):
    ...
    def post(self, request):
        """Creates or updates a portfolio by name, adding/updating companies."""
        input_serializer = PortfolioCompanyInputSerializer(data=request.data)
        ...
        # 2. Process companies list (Clear existing and add new for simplicity)
        PortfolioCompany.objects.filter(portfolio=portfolio).delete()
        
        company_data_list = data['companies_data']
        
        for item in company_data_list:
            id_key = item['id_key']
            aum = item.get('aum', 0.0)
            
            # Identify company by ISIN (12 characters) or Name
            if len(id_key) == 12 and id_key.startswith('IN'):
                company_query = Company.objects.filter(isin__iexact=id_key)
            else:
                company_query = Company.objects.filter(company_name__iexact=id_key)
            
            company = company_query.first()
            
            if company:
                PortfolioCompany.objects.create(
                    portfolio=portfolio,
                    company=company,
                    aum_value=aum
                )
...
class PortfolioCompanyUpdateView(APIView):
    permission_classes = [IsAuthenticated]

    def patch(self, request, pk):
        """Updates the AUM for a single PortfolioCompany holding."""
        aum_value = request.data.get('aum_value')
        
        holding = get_object_or_404(
            PortfolioCompany.objects.filter(portfolio__user=request.user), 
            pk=pk
        )
        ...
        if aum_value is not None:
            try:
                ...
                holding.aum_value = aum_value
                holding.save()
                return Response(PortfolioCompanySerializer(holding).data)
            except ValueError:
                return Response({"error": "AUM value must be a valid number."}, status=status.HTTP_400_BAD_REQUEST)

        return Response({"message": "No fields updated."}, status=status.HTTP_200_OK)

    def delete(self, request, pk):
        """Deletes a single PortfolioCompany holding from a portfolio."""
        holding = get_object_or_404(
            PortfolioCompany.objects.filter(portfolio__user=request.user), 
            pk=pk
        )
