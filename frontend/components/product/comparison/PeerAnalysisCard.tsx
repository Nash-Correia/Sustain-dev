...
function FundsSection({
  ...
  allCompanyData,
  fundCompanies,      // NEW
  fundCompanyIsins,   // NEW
}: {
  fundName: string;
  allCompanyData: CompanyDataRow[];
  fundCompanies?: CompanyDataRow[];
  fundCompanyIsins?: string;
}) {
  // Resolve live selection:
  // 1) If an explicit array was provided, use it.
  // 2) Else if ISIN CSV provided, map it to rows from allCompanyData.
  // 3) Else -> empty.
  const selectionCompanies: CompanyDataRow[] = React.useMemo(() => {
    if (fundCompanies && fundCompanies.length) return fundCompanies;

    if (fundCompanyIsins && fundCompanyIsins.trim()) {
      const map = new Map<string, CompanyDataRow>();
      for (const c of allCompanyData) {
        if (c.isin) map.set(c.isin.trim().toUpperCase(), c);
      }
      const uniqIsins = Array.from(
        new Set(
          fundCompanyIsins
            .split(",")
      ...
      for (const isin of uniqIsins) {
        const row = map.get(isin);
    ...
    return [];
  }, [fundCompanies, fundCompanyIsins, allCompanyData]);

  // If no companies, hide the card entirely
  if (!selectionCompanies.length) return null;
  ...
  return (
    <div className="bg-white shadow-sm p-6 sm:p-8 mb-8"
    >
      <section>
          ...
          <StatCard
            label={
              <>
                ...
                <InfoTooltip
                  id="fundTopHoldingCompositeScore"
                  align="center"
                  panelWidthClass="w-72"
                />
              </>
...
export default function AnalysisCard({
  ...
  if (selectedItem.type === "Funds") {
    return (
      <FundsSection fundName={selectedItem.name} allCompanyData={allCompanyData}         fundCompanies={fundCompanies}
        fundCompanyIsins={fundCompanyIsins} />
    );
  }
