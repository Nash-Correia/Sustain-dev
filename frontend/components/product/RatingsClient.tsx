    ...
    (async () => {
      ...
      try {
        ...
        if (companies.length > 0) {
          ...
          console.log(`🧩 First company top-level keys: ${topKeys}`);
        }

        // Count how many we can extract a rating for
        const foundCount = companies.reduce((acc, c) => (pickRatingFlexible(c) ? acc + 1 : acc), 0);
        console.log(
          `ℹ️ Ratings identified via flexible extractor: ${foundCount} of ${companies.length}`
        );
        ...
        const ratingRows: RatingRow[] = companies
          ...
          .map((c) => {
            const ratingValue = pickRatingFlexible(c) ?? "—";
            return {
